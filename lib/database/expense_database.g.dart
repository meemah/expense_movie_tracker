    ...
    final databaseOptions = sqflite.OpenDatabaseOptions(
      ...
      onCreate: (database, version) async {
        await database.execute(
            'CREATE TABLE IF NOT EXISTS `Expense` (`id` INTEGER PRIMARY KEY AUTOINCREMENT, `expenseTitle` TEXT, `amount` REAL, `currentDate` TEXT)');
  ...
  Stream<List<Expense>> findAllExpenses() {
    return _queryAdapter.queryListStream('SELECT * FROM Expense',
        ...
        mapper: (Map<String, dynamic> row) => Expense(
            row['id'] as int,
            row['expenseTitle'] as String,
            row['amount'] as double,
            row['currentDate'] as String));
  }

  @override
  Stream<double> calculateTotal() async* {
    AppDatabase _database =
        await $FloorAppDatabase.databaseBuilder('expense_database').build();
    final numbersSum =
        await _database.database.rawQuery('SELECT SUM(amount) FROM EXPENSE');
    print(numbersSum[0]["SUM(amount)"].toString());
    yield numbersSum[0]["SUM(amount)"];
    // await _queryAdapter.query('SELECT SUM(amount) FROM Expense', mapper: null);
    // await _queryAdapter.queryNoReturn('SELECT SUM(amount) FROM Expense');
  }
